...
/**
 * Main screen composable for displaying the payloads list.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun PayloadsScreen(
    navController: NavController,
    viewModel: PayloadsViewModel = hiltViewModel(),
    ...
    DisposableEffect(Unit) {
        sharedViewModel.registerRefreshHandler {
            viewModel.onEvent(PayloadsEvent.Refresh)
        }
            ...
            state.hasError -> {
                Box(
                    ...
                    ErrorMessage(
                        ...
                        onRetry = { viewModel.onEvent(PayloadsEvent.Retry) }
                    )
                }
            }
            else -> {
