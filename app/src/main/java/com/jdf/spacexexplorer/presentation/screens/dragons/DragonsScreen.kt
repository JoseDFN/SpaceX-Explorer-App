...
/**
 * Main screen composable for displaying the dragons list.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun DragonsScreen(
    navController: NavController,
    viewModel: DragonsViewModel = hiltViewModel(),
    ...
    DisposableEffect(Unit) {
        sharedViewModel.registerRefreshHandler {
            viewModel.onEvent(DragonsEvent.Refresh)
        }
            ...
            state.hasError -> {
                Box(
                    ...
                    ErrorMessage(
                        ...
                        onRetry = { viewModel.onEvent(DragonsEvent.Retry) }
                    )
                }
            }
            else -> {
                    ...
                    LazyColumn(
                        ...
                        items(state.dragons) { dragon ->
                            DragonCard(
                                dragon = dragon,
                                onClick = {
                                    viewModel.onEvent(DragonsEvent.DragonClicked(dragon.id))
                                }
                            )
                        }
                    }
