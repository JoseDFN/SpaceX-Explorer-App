...
/**
 * Main screen composable for displaying the launchpads list.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun LaunchpadsScreen(
    ...
    DisposableEffect(Unit) {
        sharedViewModel.registerRefreshHandler {
            viewModel.onEvent(LaunchpadsEvent.Refresh)
        }
            ...
            state.hasError -> {
                Box(
                    ...
                    ErrorMessage(
                        ...
                        onRetry = { viewModel.onEvent(LaunchpadsEvent.Retry) }
                    )
                }
            }
            else -> {
                LazyColumn(
                    ...
                    items(state.launchpads) { launchpad ->
                        LaunchpadCard(
                            launchpad = launchpad,
                            onClick = {
                                viewModel.onEvent(LaunchpadsEvent.LaunchpadClicked(launchpad))
                            }
                        )
                    }
                }
